...
import { aiOutputPlayerProps } from "@/shared/types";
import { formatDuration } from "@/helpers/archive-ai-files-functions";

const AiOutputPlayer = ({
  currentTab,
  paused,
  audioRef,
  duration,
  ...
  // set main audio duration with personal audio duration slider
  useEffect(() => {
    if (!paused) {
  ...
  // set main audio volume with personal audio volume slider
  ...
  // change audio position/value on slider change
  const positionHandler = (value: number) => {
    setPosition(value);
    if (audioRef.current) {
    ...
    <div className='ai-output-player'>
      ...
      <div className='slider'>
        <Slider
          ...
          onChange={(_, value) => positionHandler(value as number)}
          sx={{
            color:
              currentTab === "record"
                ? "#00ba9f"
      ...
      <div className='voice-time'>
        {!position
          ...
          : formatDuration(position as number)}
      </div>
