...
import { aiOutputProps } from "@/shared/types";
import AiOutputPlayer from "./ai-ouput-player/AiOutputPlayer";
import SimpleText from "./simple-text/SimpleText";
import TimeText from "./time-text/TimeText";
import { restartIcon } from "@/utilities/svg/svg";
import {
  copyToClipboard,
  downloadHandler,
  restartHandler,
...
const AiOutput = ({
  ...
  return (
    ...
    >
      <div className="ai-output-top">
        {/* ai output top left side */}
        <div className="ao-top-left">
          <div
            className={
              currentTab === "record"
                ? "ao-top-left-restart restart-record"
                : currentTab === "upload"
                ? "ao-top-left-restart restart-upload"
                : currentTab === "link"
                ? "ao-top-left-restart restart-link"
                : ""
            }
            onClick={() => restartHandler({ setIsFetch, setPaused, audioRef })}
          >
            <span className="ao-top-left-restart-text">شروع دوباره</span>
            {/* restart icon */}
            {restartIcon}
          </div>
          {/* copy tooltip container - mui */}
          <ClickAwayListener
            ...
            <div>
              ...
              >
                ...
                <svg
                  ...
                  onClick={() => {
                    copyToClipboard(text), tooltipOpenHandler({ setOpen });
                  }}
                >
                  <path
        ...
        {/* ai output top right side */}
        <div className="ao-top-right">
          <div
            className={
              activeText === 2
                ? "ao-top-right-time-text active-text"
                : "ao-top-right-time-text"
            }
          ...
          <div
            className={
              activeText === 1
                ? "ao-top-right-simple-text  active-text"
                : "ao-top-right-simple-text"
            }
      ...
      {/* ai texts output */}
      <div className="ai-output-center">
        {activeText === 1 ? (
          <SimpleText lang={lang} text={text} />
        ) : (
          <TimeText paused={paused} timeText={timeText} />
        )}
      </div>
      {/* ai voice output */}
      <div className="ai-output-bottom">
        <AiOutputPlayer
          setPaused={setPaused}
          paused={paused}
